...
/*   Updated: 2024/01/22 17:23:23 by jcoquard         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	raycast(t_data *cub)
{
	...
	while (i < cub->win.w)
	{
		...
		map_x = (int)(cub->pl.x / map->tile_dim);
		map_y = (int)(cub->pl.y / map->tile_dim);
		if (direction_ray_x < 0)
		{
			step_x = -1;
			next_x = (cub->pl.x / map->tile_dim - map_x) * range_next_x;
		}
		else
		{
			step_x = 1;
			next_x = (map_x + 1 - cub->pl.x / map->tile_dim) * range_next_x;
		}
		if (direction_ray_y < 0)
		{
			step_y = -1;
			next_y = (cub->pl.y / map->tile_dim - map_y) * range_next_y;
		}
		else
		{
			step_y = 1;
			next_y = (map_y + 1 - cub->pl.y / map->tile_dim) * range_next_y;
		}
...
//		if(side == 0)
//			perpWallDist.push_back(prochainX - distanceProchainX);
//		else
//			perpWallDist.push_back(prochainY - distanceProchainY);
		if (collision)
		{
			//correction de l'effet fish-eye
			if (side == 0)
				distance = fabs((map_x - (cub->pl.x / map->tile_dim) + (1 - step_x) / 2) / direction_ray_x);
			else
				distance = fabs((map_y - (cub->pl.y / map->tile_dim) + (1 - step_y) / 2) / direction_ray_y);
			...
			return Line(x,drawStart,x,drawEnd);
		}
		i++;
	}
}
